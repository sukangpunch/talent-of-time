...
public class ClassSessionNotificationScheduler {
    ...
    // 11시 31분부터 매 2분 간격으로 실행 (23:31, 23:33, 23:35 ... 23:59)
    @Scheduled(cron = "0 31/2 23,0 * * *")    public void notifyTestEntry() {
        sendToOnDutyCrews(LocalTime.now(), "수업 알림", "Test교시 입실 시작 10분 전입니다.");
    }

    // 11시 32분부터 매 2분 간격으로 실행 (23:32, 23:34, 23:36 ... 23:58)
    @Scheduled(cron = "0 31/2 23,0,1,2 * * *")    public void notifyTestExit() {
    ...
    private void sendToOnDutyCrews(
        ...
        List<Crew> crews = crewRepository.findAllByFcmTokenIsNotNullAndCrewTypeIn(onDutyTypes);
        if (crews.isEmpty()) {
        ...
        for (Crew crew : crews) {
            try {
                firebaseCloudMessageService.sendMessage(
                        new FcmDataRequest(crew.getFcmToken(), title, body)
                );
            } catch (Exception e) {
                log.error("FCM 전송 실패: crewId={}, error={}", crew.getId(), e.getMessage());
            }
        }
        log.info("알림 전송 완료: body={}, 대상 크루 {}명 (근무 유형: {})", body, crews.size(), onDutyTypes);
    }
}
