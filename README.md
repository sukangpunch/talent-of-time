...
> 학원 알바생(크루)의 일일 업무 스케줄을 **Count 기반 공평 배정 알고리즘**으로 자동화하는 백엔드 서버
...
크루 유형별 출퇴근 시간에 따라 교시·작업마다 배정 가능한 풀이 다릅니다.

| 교시 | 작업 | 배정 풀 |
|------|------|---------|
| 세팅 | SETTING | 오전 only |
| 1교시 | ENTRY, JOG | 오전 only |
| 1교시 | EXIT | 오전 + 미들 |
| 2교시 | ENTRY, JOG, EXIT | 오전 + 미들 |
| 3교시 | ENTRY, JOG | 오전 + 미들 |
| 3교시 | EXIT | 오후 only |
| 4교시 | ENTRY, JOG, EXIT | 미들 + 오후 |
| 5·6교시 | ENTRY, JOG, EXIT | 오후 only |

### 배정 순서

```
1. 해당 교시+작업의 배정 풀 내 크루 목록 조회
2. 작업 유형별 count가 가장 낮은 크루 선택
3. count 동일 시 → 이름 오름차순
4. 동일 교시 내 이미 배정된 크루(busyCrewIds)는 후순위
5. 배정 확정 시 해당 크루의 count +1
```
